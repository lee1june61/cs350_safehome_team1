...
def load_image(canvas: tk.Canvas, w: int, h: int):
    """Load and display floor plan image at fixed size (607x373). Returns (PhotoImage, x_offset, y_offset, scale_x, scale_y) or None.

    Image is displayed at original size (607x373) and positioned at top-left (0, 0).
    This allows using hardcoded pixel coordinates directly without transformation.
    """
    img_path = find_image()
    ...
    try:
        ...
        # Fixed image dimensions (original size)
        FIXED_IMG_WIDTH = 607
        FIXED_IMG_HEIGHT = 373

        # Display image at fixed size (no scaling)
        photo = ImageTk.PhotoImage(img)
        # Position at top-left corner (0, 0)
        x_off, y_off = 0, 0
        canvas.create_image(x_off, y_off, image=photo, anchor="nw")

        # Return fixed transformation: no offset, scale = 1.0
        # This allows using hardcoded pixel coordinates directly
        return photo, x_off, y_off, 1.0, 1.0
    except Exception:
        draw_fallback(canvas, w, h)
        return None
...
def draw_device(
    ...
    canvas.tag_bind(tag, "<Button-1>", lambda e, d=dev_id, t=dtype: click_handler(d, t))
