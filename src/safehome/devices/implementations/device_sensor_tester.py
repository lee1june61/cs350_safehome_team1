...
class DeviceSensorTester(ABC):
    ...
    def showSensorTester():
        ...
        try:
            ...
            # Import from virtual_device_v3 for now - can be migrated later
            # TODO: Copy safehome_sensor_test_gui.py to src if needed
            try:
                from virtual_device_v3.virtual_device_v3.device.safehome_sensor_test_gui import SafeHomeSensorTest
            except ImportError:
                print("Warning: SafeHomeSensorTest GUI not available")
                return
